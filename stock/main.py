...
frame = "pytorch" # 使用PyTorch框架
from model import train, predict

# 配置类
class Config:
    # 数据参数
    feature_columns = list(range(3,8))     # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
    label_columns = [4, 5, 6]              # 要预测的列，按原数据从0开始计算, 如同时预测第四，五，六列 最高价，最低价，收盘价
    ...
    time_step = 50              # 设置用前多少天的数据来预测，也是LSTM的time step数，请保证训练数据量大于它
    ...
    epoch = 20                  # 整个训练集最多被训练多少遍
    patience = 5                # 训练5个epoch 验证集没提升就停掉
    ...
    do_figure_save = True               # 是否将figure保存
    do_train_visualized = True          # 训练loss可视化，pytorch用visdom
...
class Data:
    def __init__(self, config):
        ...
        self.train_num = int(self.data_num * self.config.train_data_rate) # 训练数据量

        # 预处理
        self.mean = np.mean(self.data, axis=0)              # 均值
        self.std = np.std(self.data, axis=0)                # 方差
    ...
    def get_test_data(self, return_label_data=False):
        ...
        self.start_num_in_test = feature_data.shape[0] % sample_interval  # 这些天的数据不够一个sample_interval，要去除
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    label_data = origin_data.data[origin_data.train_num + origin_data.start_num_in_test : ,
                                            config.label_in_feature_index]   # 实际数据
...
if __name__=="__main__":
    con = Config()
    import argparse
    # argparse方便于命令行下输入参数，可以根据需要增加更多，要用时将以下注释打开。如命令行输入python main.py -t True -p False 则会开始训练
